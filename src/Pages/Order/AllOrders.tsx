...
function AllOrders() {
  ...
  useEffect(() => {
    if (data) {
      ...
      const responseData = data.apiResponse?.result || data.result || data;
      setOrderData(responseData);
      
      // Also handle the totalRecords parsing more safely
      try {
        const { TotalRecords } = data.totalRecords 
          ? JSON.parse(data.totalRecords) 
          : { TotalRecords: responseData?.length || 0 };
        setTotalRecords(TotalRecords);
      } catch (error) {
        console.error("Error parsing totalRecords:", error);
